...
async def stream_fusion(
    ...
    *,
    ...
    """流式 fusion 的对外入口：去重面板 key 后，把整条链路包进一个 "fusion" 父 span。

    web 为必填（无默认）：联网是高成本/高延迟操作，强制调用方显式声明，避免不知情时触发大量请求。
    薄壳只负责建父 span + 透传事件，真正编排在 _stream_fusion_body——这样无需给整个异步生成器重新缩进；
    且 create_task 在 span 作用域内捕获 OTel 上下文，面板任务里的自动 LLM span 会自动挂到 fusion 之下。
    """
    panel_keys = _dedup_keys(panel_keys)
    with span("fusion", panel=",".join(panel_keys), web=web):
        async for ev in _stream_fusion_body(prompt, panel_keys, cfg, web, registry, strategy):
            yield ev


async def _stream_fusion_body(prompt, panel_keys, cfg, web, registry, strategy):
    """流式 fusion 编排体：并行面板（只上报状态事件）-> 裁判（非流式）-> 合成（流式）。
    ...
    panel_keys 已由 stream_fusion 去重。
    """
    ...
    async def run_web(ci):
        ...
        with span(f"panel:{ci.key}", model=ci.name, mode="web"):
            try:
    ...
    async def run_offline(ci):
        t0 = time.perf_counter()
        with span(f"panel:{ci.key}", model=ci.name, mode="offline"):
            try:
    ...
    while done < len(tasks):
        ...
        if isinstance(ev, (ToolStarted, ToolFinished, AgentDegraded, AgentDone,
                           ReflexionStarted, ReflexionDone)):
            yield ev
    ...
    yield JudgeStarted()
    judge_err = None
    with span("judge", model=cfg.judge_model):
        try:
    ...
    if judge_err:  # StageError 在 span 外 yield（避免在 span 作用域内挂起生成器）
        yield StageError("judge", judge_err)
    yield JudgeFinished(analysis)

    yield SynthesisStarted()
    synth_err = None
    with span("synthesize", model=cfg.synthesizer_model):
        try:
            async for piece in stream_synthesize(
                prompt, analysis, ok, cfg.get(cfg.synthesizer_model)
            ):
                yield SynthesisDelta(piece)  # 逐字 delta 必须在 span 内：这正是要观测的合成耗时
        except Exception as e:
            synth_err = str(e)
    if synth_err:
        yield StageError("synth", synth_err)
